...
do
    ...
    FILESIZE=`du -k "$currentWordlist" | cut -f1`
    if [[ "$FILESIZE" -gt "0" && "$FILESIZE" -le "3145728" ]];
    then
        wordlist=$(basename "$currentWordlist" | sed 's/\(.*\)\..*/\1/')
        suffix="_small_straight.magcracked"
        seperator="_"
    ...
    then
        wordlist=$(basename "$currentWordlist" | sed 's/\(.*\)\..*/\1/')
        suffix="_medium_straight.magcracked"
        seperator="_"
        cracked_file=$OutputFolder$wordlist$suffix
        ./hashcat -m "$HashType" -w 3 --remove "$HashesFile" "$currentWordlist" -r "$RULE" -o "$cracked_file" -O
        ProfileMedium
    ...
    then
        wordlist=$(basename "$currentWordlist" | sed 's/\(.*\)\..*/\1/')
        suffix="_large_straight.magcracked"
        seperator="_"
        cracked_file=$OutputFolder$wordlist$suffix
        ./hashcat -m "$HashType" -w 3 --remove "$HashesFile" "$currentWordlist" -r "$RULE" -o "$cracked_file" -O
        ProfileLarge
